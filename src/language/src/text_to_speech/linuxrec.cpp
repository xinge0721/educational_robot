...
static int list_pcm(snd_pcm_stream_t stream, char** name_out, char** desc_out)
{
    void **hints, **n;
    char **name, **descr;
    char *io;
    ...
    filter = (stream == SND_PCM_STREAM_CAPTURE) ? "Input" : "Output";  // const char*
    ...
    while (*n != NULL && i < cnt) {
        ...
        *descr = snd_device_name_get_hint(*n, "DESC");
        ...
        if (*name == NULL || (io != NULL && strcmp(io, filter) != 0)) {
            if (*name) free(*name);
            if (*descr) free(*descr);
        } else {
            if (*descr == NULL) {
                *descr = (char*)malloc(128);  // 分配足够空间
                memset(*descr, 0, 128);      // 初始化内存
            }
            name++;
            descr++;
            i++;
